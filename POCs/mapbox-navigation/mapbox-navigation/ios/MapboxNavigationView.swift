...
class MapboxNavigationView: UIView, NavigationViewControllerDelegate {
  ...
  var embedded: Bool
  var embedding: Bool
  weak var pointAnnotationManager: PointAnnotationManager?
  ...
  private func createPointAnnotations() {
    for annotation in self.points {
        let convertedAnnotation = annotation as! [String: Any]
        let lat = convertedAnnotation["lat"] as! CLLocationDegrees
        let long = convertedAnnotation["long"] as! CLLocationDegrees
        let image = RCTConvert.uiImage(convertedAnnotation["image"])!
        
        var pointAnnotation = PointAnnotation(coordinate: CLLocationCoordinate2DMake(lat, long))
        pointAnnotation.image = .init(image: image, name: "point")
        pointAnnotation.iconAnchor = .bottom
              
              
        if(self.pointAnnotationManager == nil) {
            self.pointAnnotationManager = self.navViewController?.navigationMapView!.mapView.annotations.makePointAnnotationManager()
        }
